...
- **Resolved: `PlayerProfileOverlay`'s stats are now reliably season-scoped.**
  ...
  only 2 league matches all season). Goals/assists for the **Top Scorers**
  view (`syncTopScorers.js`) were already fixed this way by aggregating from
  `match_events` instead, which is reliably current-season since fixtures are
  only ever synced for the season in progress. `syncPlayerSeasonStats.js`
  (daily workflow) now applies the same aggregation -- plus yellow/red cards,
  also present in `match_events` -- to every tracked club's own squad, not
  just the top scorers, and `PlayerProfileOverlay` shows only those four
  counts. Went with option (2) from this entry's prior research rather than
  keeping the rest with an "unverified" caveat: every other GOAL-API-only
  category (minutes, rating, shots, passes, key passes, tackles,
  interceptions, duels won, dribbles succeeded, saves, inside-box saves,
  goals conceded) has no season-clean source in this project's own data and
  is hidden entirely rather than shown next to reliable numbers with no way
  to tell the two apart. Matches played is deliberately not included either:
  unlike goals/assists/cards it has no event-based ground truth (a full,
  unremarkable 90 minutes emits no `match_event` row at all).
  Re-researched free-tier alternatives once more (2026-09, before landing on
  the match_events fix above): football-data.org (already integrated) gates
  player-level match stats behind its paid "Deep Data" add-on; SportMonks'
  free tier only covers the Danish Superliga and Scottish Premiership, not
  our five leagues; TheSportsDB is crowd-sourced and too inconsistent;
  API-Football was already ruled out below; soccerway.com scrapes without
  being blocked but its HTML didn't reliably expose position/photo/club/
  transfer-table fields; fbref.com returns a real Cloudflare challenge page
  to a plain HTTP fetch (its Sports Reference ToS also prohibits scraping
  outright) -- confirmed no free source exists for season-clean *detailed*
  stats (shots/passes/tackles/...), only for the goals/assists/cards our own
  match_events already covers exactly.
