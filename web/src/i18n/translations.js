// One nested object per language, not a flat "a.b.c" string-key lookup --
// components just destructure the slice they need off `t` (e.g.
// `t.settings.language`), which is plain JS and needs no path-parsing
// helper. Leaves that need a variable (relative time, "Xth matchday", an
// aria-label naming a specific club) are functions instead of strings,
// called as `t.transfers.daysAgo(3)`.
//
// Never translated here (kept as-is by the calling component instead):
// club/player names, league names (Serie A, Bundesliga, Premier League,
// Ligue 1, LaLiga -- official spellings per lib/leagues.js), source names
// (kicker, tuttomercatoweb, marca, ...), and the "KickRadar" brand name.

// English ordinal suffix (1st, 2nd, 3rd, 4th, 11th, 21st, ...) -- used by
// en.stats.positionLabel. The other languages' ordinal marks are fixed
// (German ".", Italian "°", Spanish "º") or a simple 1-vs-rest split
// (French "er"/"e"), so only English needs real logic.
function ordinalEn(n) {
  const v = n % 100;
  if (v >= 11 && v <= 13) return `${n}th`;
  switch (n % 10) {
    case 1:
      return `${n}st`;
    case 2:
      return `${n}nd`;
    case 3:
      return `${n}rd`;
    default:
      return `${n}th`;
  }
}

const de = {
  nav: { news: 'News', ligen: 'Ligen', live: 'Live', europa: 'Europa', settings: 'Einstellungen', fixtures: 'Spiele', standings: 'Tabelle' },
  common: {
    loading: 'Lädt…',
    chooseClub: 'Verein wählen…',
    notSupported: 'nicht unterstützt',
    vs: 'vs',
    cancel: 'Abbrechen',
    refreshing: 'Aktualisiert…',
    releaseToRefresh: 'Loslassen zum Aktualisieren',
    pullToRefresh: 'Zum Aktualisieren ziehen',
  },
  transfers: {
    officialOnly: 'Nur offizielle Transfers',
    officialOnlyToggle: 'Nur offizielle Transfers umschalten',
    empty: 'Keine Meldungen für diese Auswahl.',
    official: 'offiziell',
    rumor: 'gerücht',
    searchPlayer: 'Spieler suchen',
    searchPlayerTitle: 'Spieler auf Transfermarkt suchen',
    viewProfile: 'Profil ansehen',
    viewProfileTitle: 'Spielerprofil ansehen',
    // "AI Summary" itself is never translated (kept literal in every
    // language, per explicit call) -- only the overlay's other text is.
    readOriginal: 'Artikel lesen',
    aiSummaryDisclaimer: 'KI-generierte Zusammenfassung, kann Fehler enthalten. Ersetzt nicht den Originalartikel.',
    justNow: 'gerade eben',
    minutesAgo: (n) => `vor ${n} Min`,
    hoursAgo: (n) => `vor ${n} Std`,
    daysAgo: (n) => `vor ${n} Tag${n === 1 ? '' : 'en'}`,
  },
  newsSection: { tabNews: 'News', tabTransfers: 'Transfers' },
  fixtures: {
    currentMatchdayOnly: 'Nur aktueller Spieltag',
    currentMatchdayOnlyToggle: 'Nur aktuellen Spieltag anzeigen umschalten',
    liveOnlyToggle: 'Nur laufende Spiele anzeigen umschalten',
    todayOnlyToggle: 'Nur heutige Spiele anzeigen umschalten',
    empty: 'Keine Spiele im Kalender.',
    matchday: (n) => `${n}. Spieltag`,
    live: 'LIVE',
    today: 'HEUTE',
    finished: 'Beendet',
    kickoffTbd: '-:-',
    favoriteAria: 'Spiel als Favorit markieren',
    unfavoriteAria: 'Favorit entfernen',
    favoritedToast: 'Als Favorit markiert',
    unfavoritedToast: 'Favorit entfernt',
  },
  lineup: {
    notYetKnown: 'Aufstellung noch nicht bekannt. Wird veröffentlicht, sobald die Vereine sie bestätigen (meist 30–60 Min vor Anpfiff).',
    official: 'Offizielle Aufstellung',
    predicted: 'Voraussichtliche Aufstellung',
    noPlayers: 'Noch keine Spieler gemeldet.',
    substitutes: 'Ersatzbank',
    coach: 'Trainer',
    positions: { Goalkeeper: 'Torwart', Defender: 'Verteidiger', Midfielder: 'Mittelfeld', Forward: 'Sturm' },
  },
  playerProfile: {
    age: (n) => `${n} Jahre`,
    statsAsOf: (d) => `Stand: ${d}`,
    goals: 'Tore',
    assists: 'Vorlagen',
    yellowCards: 'Gelbe Karten',
    redCards: 'Rote Karten',
    injured: 'Verletzt',
    noStats: 'Keine Statistiken verfügbar.',
  },
  matchInfo: {
    tabLineups: 'Aufstellungen',
    tabInfo: 'Spielinfo',
    tabStats: 'Statistiken',
    tabTable: 'Tabelle',
    tabHighlights: 'Highlights',
    noHighlights: 'Highlights noch nicht verfügbar.',
    notFinished: 'Die Ereignisübersicht ist verfügbar, sobald das Spiel beendet ist.',
    noEvents: 'Keine nennenswerten Ereignisse.',
    goal: 'Tor',
    ownGoal: 'Eigentor',
    penalty: 'Elfmeter',
    yellowCard: 'Gelbe Karte',
    redCard: 'Rote Karte',
    substitution: 'Wechsel',
    assistLabel: (name) => `Vorlage: ${name}`,
  },
  standings: {
    title: 'Tabelle',
    empty: 'Tabelle noch nicht verfügbar.',
    played: 'Sp',
    won: 'S',
    draw: 'U',
    lost: 'N',
    points: 'Pkt',
    goalDiff: 'TD',
    zoneChampionsLeague: 'Champions League',
    zoneEuropaLeague: 'Europa League',
    zoneConferenceLeague: 'Conference League',
    zoneRelegationPlayoff: 'Relegation',
    zoneRelegation: 'Abstieg',
  },
  europa: {
    tabSpiele: 'Spiele',
    tabTabelle: 'Tabelle',
    leaguePhase: 'Ligaphase',
    zoneDirectQualify: 'Direktes Achtelfinale',
    zonePlayoff: 'Playoff-Runde',
    zoneEliminated: 'Ausgeschieden',
    standingsEmpty: 'Tabelle noch nicht verfügbar.',
  },
  live: {
    title: 'Live',
    empty: 'Gerade läuft kein Spiel',
    emptyHint: 'Schau später wieder vorbei, oder wirf einen Blick auf den Spielplan unter Ligen oder Europa.',
  },
  topscorers: {
    title: 'Torschützen',
    empty: 'Torschützen-Ranking noch nicht verfügbar.',
    goals: 'Tore',
    assists: 'Vorl',
  },
  stats: {
    form: 'Form',
    noForm: 'Noch keine Spiele in dieser Saison.',
    headToHead: 'Direkter Vergleich',
    noHeadToHead: 'Noch kein Duell in dieser Saison.',
    standing: 'Tabellenplatz',
    positionLabel: (pos) => `${pos}.`,
    possession: 'Ballbesitz',
    shotsOnGoal: 'Schüsse aufs Tor',
    shotsOffGoal: 'Schüsse neben das Tor',
    shotsBlocked: 'Abgewehrte Schüsse',
    corners: 'Ecken',
    offsides: 'Abseits',
    fouls: 'Fouls',
    yellowCards: 'Gelbe Karten',
    redCards: 'Rote Karten',
    noStatistics: 'Noch keine Statistiken für dieses Spiel.',
  },
  settings: {
    appearance: 'Darstellung',
    appearanceSystem: 'System',
    appearanceLight: 'Hell',
    appearanceDark: 'Dunkel',
    accentColor: 'Akzentfarbe',
    accentTerracotta: 'Terracotta',
    accentViolet: 'Lila',
    accentGreen: 'Grün',
    accentMono: 'Mono',
    favoriteClub: 'Lieblingsverein',
    quickFilters: 'Quick-Filter',
    noQuickFilters: 'Noch keine Quick-Filter angelegt. Füge sie im Transfers-Tab hinzu.',
    removeClub: (name) => `${name} entfernen`,
    notifications: 'Benachrichtigungen',
    pushTransfers: 'Push bei Transfer-News',
    pushTransfersToggle: 'Push bei Transfer-News umschalten',
    pushLineups: 'Push bei offizieller Aufstellung',
    pushLineupsToggle: 'Push bei offizieller Aufstellung umschalten',
    language: 'Sprache',
    legal: 'Rechtliches',
    imprint: 'Impressum',
    privacyPolicy: 'Datenschutz',
    dataSource: 'Datenquellen',
    dataSourceAttribution: 'Football data provided by the Football-Data.org API. Weitere Daten: GOAL API.',
  },
  quickFilters: {
    addAria: 'Quick-Filter hinzufügen',
    confirmRemove: 'Willst du den Filter wirklich entfernen?',
    remove: 'Entfernen',
  },
  clubDetail: {
    tabSquad: 'Kader',
    tabFixtures: 'Spiele',
    tabTransfers: 'Transfers',
    squadUnavailable: 'Kader aktuell nicht verfügbar.',
    squadEmpty: 'Kein Kader gemeldet.',
    founded: (year) => `Gegründet ${year}`,
    capacity: (n) => `${n} Plätze`,
    captain: 'Kapitän',
    injured: 'Verletzt',
    noFixtures: 'Keine anstehenden Spiele.',
    noTransfers: 'Keine Transfer-News für diesen Verein.',
  },
  errors: {
    notificationsDenied: 'Benachrichtigungen wurden nicht erlaubt.',
    tabCrashed: 'Etwas ist schiefgelaufen.',
    retry: 'Erneut versuchen',
  },
};

const en = {
  nav: { news: 'News', ligen: 'Leagues', live: 'Live', europa: 'Europe', settings: 'Settings', fixtures: 'Fixtures', standings: 'Table' },
  common: {
    loading: 'Loading…',
    chooseClub: 'Choose club…',
    notSupported: 'not supported',
    vs: 'vs',
    cancel: 'Cancel',
    refreshing: 'Refreshing…',
    releaseToRefresh: 'Release to refresh',
    pullToRefresh: 'Pull to refresh',
  },
  transfers: {
    officialOnly: 'Official transfers only',
    officialOnlyToggle: 'Toggle official transfers only',
    empty: 'No reports for this selection.',
    official: 'official',
    rumor: 'rumor',
    searchPlayer: 'Search player',
    searchPlayerTitle: 'Search player on Transfermarkt',
    viewProfile: 'View profile',
    viewProfileTitle: 'View player profile',
    readOriginal: 'Read article',
    aiSummaryDisclaimer: 'AI-generated summary, may contain errors. Not a substitute for the original article.',
    justNow: 'just now',
    minutesAgo: (n) => `${n} min ago`,
    hoursAgo: (n) => `${n} hr ago`,
    daysAgo: (n) => `${n} day${n === 1 ? '' : 's'} ago`,
  },
  newsSection: { tabNews: 'News', tabTransfers: 'Transfers' },
  fixtures: {
    currentMatchdayOnly: 'Current matchday only',
    currentMatchdayOnlyToggle: 'Toggle showing only the current matchday',
    liveOnlyToggle: 'Toggle showing only live matches',
    todayOnlyToggle: "Toggle showing only today's matches",
    empty: 'No games on the calendar.',
    matchday: (n) => `Matchday ${n}`,
    live: 'LIVE',
    today: 'TODAY',
    finished: 'Finished',
    kickoffTbd: '-:-',
    favoriteAria: 'Mark match as favorite',
    unfavoriteAria: 'Remove favorite',
    favoritedToast: 'Marked as favorite',
    unfavoritedToast: 'Favorite removed',
  },
  lineup: {
    notYetKnown: 'Lineup not yet known. Published as soon as the clubs confirm it (usually 30–60 min before kickoff).',
    official: 'Official lineup',
    predicted: 'Predicted lineup',
    noPlayers: 'No players reported yet.',
    substitutes: 'Substitutes',
    coach: 'Coach',
    positions: { Goalkeeper: 'Goalkeeper', Defender: 'Defender', Midfielder: 'Midfielder', Forward: 'Forward' },
  },
  playerProfile: {
    age: (n) => `${n} years old`,
    statsAsOf: (d) => `As of ${d}`,
    goals: 'Goals',
    assists: 'Assists',
    yellowCards: 'Yellow Cards',
    redCards: 'Red Cards',
    injured: 'Injured',
    noStats: 'No stats available.',
  },
  matchInfo: {
    tabLineups: 'Lineups',
    tabInfo: 'Match Info',
    tabStats: 'Stats',
    tabTable: 'Table',
    tabHighlights: 'Highlights',
    noHighlights: 'Highlights not yet available.',
    notFinished: 'The event timeline is available once the match has finished.',
    noEvents: 'No notable events.',
    goal: 'Goal',
    ownGoal: 'Own Goal',
    penalty: 'Penalty',
    yellowCard: 'Yellow Card',
    redCard: 'Red Card',
    substitution: 'Substitution',
    assistLabel: (name) => `Assist: ${name}`,
  },
  standings: {
    title: 'Table',
    empty: 'Table not available yet.',
    played: 'P',
    won: 'W',
    draw: 'D',
    lost: 'L',
    points: 'Pts',
    goalDiff: 'GD',
    zoneChampionsLeague: 'Champions League',
    zoneEuropaLeague: 'Europa League',
    zoneConferenceLeague: 'Conference League',
    zoneRelegationPlayoff: 'Relegation play-off',
    zoneRelegation: 'Relegation',
  },
  europa: {
    tabSpiele: 'Fixtures',
    tabTabelle: 'Table',
    leaguePhase: 'League Phase',
    zoneDirectQualify: 'Direct Round of 16',
    zonePlayoff: 'Playoff Round',
    zoneEliminated: 'Eliminated',
    standingsEmpty: 'Standings not yet available.',
  },
  live: {
    title: 'Live',
    empty: 'Nothing live right now',
    emptyHint: 'Check back later, or take a look at the schedule under Leagues or Europe.',
  },
  topscorers: {
    title: 'Top Scorers',
    empty: 'Top scorers ranking not yet available.',
    goals: 'Goals',
    assists: 'Ast',
  },
  stats: {
    form: 'Form',
    noForm: 'No matches played yet this season.',
    headToHead: 'Head-to-Head',
    noHeadToHead: 'No meetings yet this season.',
    standing: 'League Position',
    positionLabel: (pos) => ordinalEn(pos),
    possession: 'Possession',
    shotsOnGoal: 'Shots on Target',
    shotsOffGoal: 'Shots off Target',
    shotsBlocked: 'Blocked Shots',
    corners: 'Corners',
    offsides: 'Offsides',
    fouls: 'Fouls',
    yellowCards: 'Yellow Cards',
    redCards: 'Red Cards',
    noStatistics: 'No statistics yet for this match.',
  },
  settings: {
    appearance: 'Appearance',
    appearanceSystem: 'System',
    appearanceLight: 'Light',
    appearanceDark: 'Dark',
    accentColor: 'Accent color',
    accentTerracotta: 'Terracotta',
    accentViolet: 'Violet',
    accentGreen: 'Green',
    accentMono: 'Mono',
    favoriteClub: 'Favorite club',
    quickFilters: 'Quick filters',
    noQuickFilters: 'No quick filters set up yet. Add them in the Transfers tab.',
    removeClub: (name) => `Remove ${name}`,
    notifications: 'Notifications',
    pushTransfers: 'Push for transfer news',
    pushTransfersToggle: 'Toggle push for transfer news',
    pushLineups: 'Push for official lineups',
    pushLineupsToggle: 'Toggle push for official lineups',
    language: 'Language',
    legal: 'Legal',
    imprint: 'Imprint',
    privacyPolicy: 'Privacy Policy',
    dataSource: 'Data sources',
    dataSourceAttribution: 'Football data provided by the Football-Data.org API. Additional data: GOAL API.',
  },
  quickFilters: {
    addAria: 'Add quick filter',
    confirmRemove: 'Do you really want to remove this filter?',
    remove: 'Remove',
  },
  clubDetail: {
    tabSquad: 'Squad',
    tabFixtures: 'Fixtures',
    tabTransfers: 'Transfers',
    squadUnavailable: 'Squad currently unavailable.',
    squadEmpty: 'No squad reported.',
    founded: (year) => `Founded ${year}`,
    capacity: (n) => `${n} seats`,
    captain: 'Captain',
    injured: 'Injured',
    noFixtures: 'No upcoming fixtures.',
    noTransfers: 'No transfer news for this club.',
  },
  errors: {
    notificationsDenied: 'Notifications were not allowed.',
    tabCrashed: 'Something went wrong.',
    retry: 'Try again',
  },
};

const it = {
  nav: { news: 'News', ligen: 'Campionati', live: 'Live', europa: 'Europa', settings: 'Impostazioni', fixtures: 'Partite', standings: 'Classifica' },
  common: {
    loading: 'Caricamento…',
    chooseClub: 'Scegli squadra…',
    notSupported: 'non supportato',
    vs: 'vs',
    cancel: 'Annulla',
    refreshing: 'Aggiornamento…',
    releaseToRefresh: 'Rilascia per aggiornare',
    pullToRefresh: 'Trascina per aggiornare',
  },
  transfers: {
    officialOnly: 'Solo trasferimenti ufficiali',
    officialOnlyToggle: 'Attiva/disattiva solo trasferimenti ufficiali',
    empty: 'Nessuna notizia per questa selezione.',
    official: 'ufficiale',
    rumor: 'voce',
    searchPlayer: 'Cerca giocatore',
    searchPlayerTitle: 'Cerca il giocatore su Transfermarkt',
    viewProfile: 'Vedi profilo',
    viewProfileTitle: 'Vedi il profilo del giocatore',
    readOriginal: "Leggi l'articolo",
    aiSummaryDisclaimer: "Riepilogo generato dall'IA, può contenere errori. Non sostituisce l'articolo originale.",
    justNow: 'proprio ora',
    minutesAgo: (n) => `${n} min fa`,
    hoursAgo: (n) => `${n} ${n === 1 ? 'ora' : 'ore'} fa`,
    daysAgo: (n) => `${n} ${n === 1 ? 'giorno' : 'giorni'} fa`,
  },
  newsSection: { tabNews: 'News', tabTransfers: 'Trasferimenti' },
  fixtures: {
    currentMatchdayOnly: 'Solo giornata attuale',
    currentMatchdayOnlyToggle: 'Attiva/disattiva solo giornata attuale',
    liveOnlyToggle: 'Attiva/disattiva solo partite in corso',
    todayOnlyToggle: 'Attiva/disattiva solo partite di oggi',
    empty: 'Nessuna partita in calendario.',
    matchday: (n) => `${n}ª giornata`,
    live: 'LIVE',
    today: 'OGGI',
    finished: 'Terminata',
    kickoffTbd: '-:-',
    favoriteAria: 'Aggiungi partita ai preferiti',
    unfavoriteAria: 'Rimuovi dai preferiti',
    favoritedToast: 'Aggiunta ai preferiti',
    unfavoritedToast: 'Rimossa dai preferiti',
  },
  lineup: {
    notYetKnown: "Formazione non ancora nota. Pubblicata non appena le squadre la confermano (di solito 30–60 min prima del fischio d'inizio).",
    official: 'Formazione ufficiale',
    predicted: 'Formazione probabile',
    noPlayers: 'Nessun giocatore ancora comunicato.',
    substitutes: 'Panchina',
    coach: 'Allenatore',
    positions: { Goalkeeper: 'Portiere', Defender: 'Difensore', Midfielder: 'Centrocampista', Forward: 'Attaccante' },
  },
  playerProfile: {
    age: (n) => `${n} anni`,
    statsAsOf: (d) => `Aggiornato al ${d}`,
    goals: 'Gol',
    assists: 'Assist',
    yellowCards: 'Cartellini gialli',
    redCards: 'Cartellini rossi',
    injured: 'Infortunato',
    noStats: 'Nessuna statistica disponibile.',
  },
  matchInfo: {
    tabLineups: 'Formazioni',
    tabInfo: 'Cronaca',
    tabStats: 'Statistiche',
    tabTable: 'Classifica',
    tabHighlights: 'Highlights',
    noHighlights: 'Highlights non ancora disponibili.',
    notFinished: "La cronologia degli eventi sarà disponibile a fine partita.",
    noEvents: 'Nessun evento rilevante.',
    goal: 'Gol',
    ownGoal: 'Autogol',
    penalty: 'Rigore',
    yellowCard: 'Cartellino giallo',
    redCard: 'Cartellino rosso',
    substitution: 'Sostituzione',
    assistLabel: (name) => `Assist: ${name}`,
  },
  standings: {
    title: 'Classifica',
    empty: 'Classifica non ancora disponibile.',
    played: 'PG',
    won: 'V',
    draw: 'N',
    lost: 'P',
    points: 'Pt',
    goalDiff: 'DR',
    zoneChampionsLeague: 'Champions League',
    zoneEuropaLeague: 'Europa League',
    zoneConferenceLeague: 'Conference League',
    zoneRelegationPlayoff: 'Playoff salvezza',
    zoneRelegation: 'Retrocessione',
  },
  europa: {
    tabSpiele: 'Partite',
    tabTabelle: 'Classifica',
    leaguePhase: 'Fase di Lega',
    zoneDirectQualify: 'Ottavi di finale diretti',
    zonePlayoff: 'Play-off',
    zoneEliminated: 'Eliminato',
    standingsEmpty: 'Classifica non ancora disponibile.',
  },
  live: {
    title: 'Live',
    empty: 'Nessuna partita in corso al momento',
    emptyHint: 'Torna più tardi, oppure guarda il calendario in Campionati o Europa.',
  },
  topscorers: {
    title: 'Capocannonieri',
    empty: 'Classifica capocannonieri non ancora disponibile.',
    goals: 'Gol',
    assists: 'Ass',
  },
  stats: {
    form: 'Forma',
    noForm: 'Nessuna partita ancora giocata in questa stagione.',
    headToHead: 'Scontri diretti',
    noHeadToHead: 'Nessun precedente in questa stagione.',
    standing: 'Posizione in classifica',
    positionLabel: (pos) => `${pos}°`,
    possession: 'Possesso palla',
    shotsOnGoal: 'Tiri in porta',
    shotsOffGoal: 'Tiri fuori',
    shotsBlocked: 'Tiri parati',
    corners: "Calci d'angolo",
    offsides: 'Fuorigioco',
    fouls: 'Falli',
    yellowCards: 'Cartellini gialli',
    redCards: 'Cartellini rossi',
    noStatistics: 'Nessuna statistica ancora per questa partita.',
  },
  settings: {
    appearance: 'Aspetto',
    appearanceSystem: 'Sistema',
    appearanceLight: 'Chiaro',
    appearanceDark: 'Scuro',
    accentColor: 'Colore accento',
    accentTerracotta: 'Terracotta',
    accentViolet: 'Viola',
    accentGreen: 'Verde',
    accentMono: 'Mono',
    favoriteClub: 'Squadra del cuore',
    quickFilters: 'Filtri rapidi',
    noQuickFilters: 'Nessun filtro rapido ancora creato. Aggiungili nella scheda Trasferimenti.',
    removeClub: (name) => `Rimuovi ${name}`,
    notifications: 'Notifiche',
    pushTransfers: 'Notifiche push sul calciomercato',
    pushTransfersToggle: 'Attiva/disattiva notifiche sul calciomercato',
    pushLineups: 'Notifiche push per formazioni ufficiali',
    pushLineupsToggle: 'Attiva/disattiva notifiche per formazioni ufficiali',
    language: 'Lingua',
    legal: 'Note legali',
    imprint: 'Impressum',
    privacyPolicy: 'Privacy',
    dataSource: 'Fonti dei dati',
    dataSourceAttribution: 'Football data provided by the Football-Data.org API. Altri dati: GOAL API.',
  },
  quickFilters: {
    addAria: 'Aggiungi filtro rapido',
    confirmRemove: 'Vuoi davvero rimuovere questo filtro?',
    remove: 'Rimuovi',
  },
  clubDetail: {
    tabSquad: 'Rosa',
    tabFixtures: 'Partite',
    tabTransfers: 'Trasferimenti',
    squadUnavailable: 'Rosa non disponibile al momento.',
    squadEmpty: 'Nessuna rosa disponibile.',
    founded: (year) => `Fondato nel ${year}`,
    capacity: (n) => `${n} posti`,
    captain: 'Capitano',
    injured: 'Infortunato',
    noFixtures: 'Nessuna partita in programma.',
    noTransfers: 'Nessuna notizia di mercato per questo club.',
  },
  errors: {
    notificationsDenied: 'Le notifiche non sono state autorizzate.',
    tabCrashed: 'Qualcosa è andato storto.',
    retry: 'Riprova',
  },
};

const fr = {
  nav: { news: 'Actualités', ligen: 'Championnats', live: 'Live', europa: 'Europe', settings: 'Réglages', fixtures: 'Matchs', standings: 'Classement' },
  common: {
    loading: 'Chargement…',
    chooseClub: 'Choisir un club…',
    notSupported: 'non pris en charge',
    vs: 'vs',
    cancel: 'Annuler',
    refreshing: 'Actualisation…',
    releaseToRefresh: 'Relâcher pour actualiser',
    pullToRefresh: 'Tirer pour actualiser',
  },
  transfers: {
    officialOnly: 'Transferts officiels uniquement',
    officialOnlyToggle: 'Activer/désactiver transferts officiels uniquement',
    empty: 'Aucune actualité pour cette sélection.',
    official: 'officiel',
    rumor: 'rumeur',
    searchPlayer: 'Rechercher le joueur',
    searchPlayerTitle: 'Rechercher le joueur sur Transfermarkt',
    viewProfile: 'Voir le profil',
    viewProfileTitle: 'Voir le profil du joueur',
    readOriginal: "Lire l'article",
    aiSummaryDisclaimer: "Résumé généré par IA, peut contenir des erreurs. Ne remplace pas l'article original.",
    justNow: "à l'instant",
    minutesAgo: (n) => `il y a ${n} min`,
    hoursAgo: (n) => `il y a ${n} h`,
    daysAgo: (n) => `il y a ${n} jour${n === 1 ? '' : 's'}`,
  },
  newsSection: { tabNews: 'News', tabTransfers: 'Transferts' },
  fixtures: {
    currentMatchdayOnly: 'Seulement la journée en cours',
    currentMatchdayOnlyToggle: 'Activer/désactiver journée en cours uniquement',
    liveOnlyToggle: 'Activer/désactiver seulement les matchs en direct',
    todayOnlyToggle: "Activer/désactiver seulement les matchs d'aujourd'hui",
    empty: 'Aucun match au calendrier.',
    matchday: (n) => `${n}${n === 1 ? 're' : 'e'} journée`,
    live: 'LIVE',
    today: "AUJOURD'HUI",
    finished: 'Terminé',
    kickoffTbd: '-:-',
    favoriteAria: 'Ajouter le match aux favoris',
    unfavoriteAria: 'Retirer des favoris',
    favoritedToast: 'Ajouté aux favoris',
    unfavoritedToast: 'Retiré des favoris',
  },
  lineup: {
    notYetKnown: "Composition pas encore connue. Publiée dès que les clubs la confirment (généralement 30 à 60 min avant le coup d'envoi).",
    official: 'Composition officielle',
    predicted: 'Composition probable',
    noPlayers: "Aucun joueur communiqué pour l'instant.",
    substitutes: 'Remplaçants',
    coach: 'Entraîneur',
    positions: { Goalkeeper: 'Gardien', Defender: 'Défenseur', Midfielder: 'Milieu', Forward: 'Attaquant' },
  },
  playerProfile: {
    age: (n) => `${n} ans`,
    statsAsOf: (d) => `Au ${d}`,
    goals: 'Buts',
    assists: 'Passes déc.',
    yellowCards: 'Cartons jaunes',
    redCards: 'Cartons rouges',
    injured: 'Blessé',
    noStats: 'Aucune statistique disponible.',
  },
  matchInfo: {
    tabLineups: 'Compositions',
    tabInfo: 'Résumé',
    tabStats: 'Statistiques',
    tabTable: 'Classement',
    tabHighlights: 'Highlights',
    noHighlights: 'Highlights pas encore disponibles.',
    notFinished: 'Le fil des événements sera disponible une fois le match terminé.',
    noEvents: 'Aucun événement notable.',
    goal: 'But',
    ownGoal: 'Contre son camp',
    penalty: 'Penalty',
    yellowCard: 'Carton jaune',
    redCard: 'Carton rouge',
    substitution: 'Changement',
    assistLabel: (name) => `Passe décisive : ${name}`,
  },
  standings: {
    title: 'Classement',
    empty: 'Classement pas encore disponible.',
    played: 'J',
    won: 'G',
    draw: 'N',
    lost: 'P',
    points: 'Pts',
    goalDiff: 'Diff',
    zoneChampionsLeague: 'Ligue des champions',
    zoneEuropaLeague: 'Ligue Europa',
    zoneConferenceLeague: 'Ligue Conférence',
    zoneRelegationPlayoff: 'Barrage de relégation',
    zoneRelegation: 'Relégation',
  },
  europa: {
    tabSpiele: 'Matchs',
    tabTabelle: 'Classement',
    leaguePhase: 'Phase de Ligue',
    zoneDirectQualify: 'Huitièmes directs',
    zonePlayoff: 'Barrages',
    zoneEliminated: 'Éliminé',
    standingsEmpty: 'Classement pas encore disponible.',
  },
  live: {
    title: 'Live',
    empty: 'Aucun match en direct pour le moment',
    emptyHint: 'Reviens plus tard, ou consulte le calendrier dans Championnats ou Europe.',
  },
  topscorers: {
    title: 'Meilleurs buteurs',
    empty: 'Classement des meilleurs buteurs pas encore disponible.',
    goals: 'Buts',
    assists: 'PD',
  },
  stats: {
    form: 'Forme',
    noForm: 'Aucun match joué cette saison pour le moment.',
    headToHead: 'Confrontation directe',
    noHeadToHead: 'Aucune confrontation cette saison.',
    standing: 'Place au classement',
    positionLabel: (pos) => `${pos}${pos === 1 ? 're' : 'e'}`,
    possession: 'Possession',
    shotsOnGoal: 'Tirs cadrés',
    shotsOffGoal: 'Tirs non cadrés',
    shotsBlocked: 'Tirs contrés',
    corners: 'Corners',
    offsides: 'Hors-jeu',
    fouls: 'Fautes',
    yellowCards: 'Cartons jaunes',
    redCards: 'Cartons rouges',
    noStatistics: 'Pas encore de statistiques pour ce match.',
  },
  settings: {
    appearance: 'Apparence',
    appearanceSystem: 'Système',
    appearanceLight: 'Clair',
    appearanceDark: 'Sombre',
    accentColor: 'Couleur d’accent',
    accentTerracotta: 'Terracotta',
    accentViolet: 'Violet',
    accentGreen: 'Vert',
    accentMono: 'Mono',
    favoriteClub: 'Club préféré',
    quickFilters: 'Filtres rapides',
    noQuickFilters: "Aucun filtre rapide créé pour l'instant. Ajoutez-en dans l'onglet Transferts.",
    removeClub: (name) => `Retirer ${name}`,
    notifications: 'Notifications',
    pushTransfers: 'Notifications push pour les news transferts',
    pushTransfersToggle: 'Activer/désactiver notifications news transferts',
    pushLineups: 'Notifications push pour compositions officielles',
    pushLineupsToggle: 'Activer/désactiver notifications compositions officielles',
    language: 'Langue',
    legal: 'Informations légales',
    imprint: 'Mentions légales',
    privacyPolicy: 'Confidentialité',
    dataSource: 'Sources de données',
    dataSourceAttribution: 'Football data provided by the Football-Data.org API. Autres données : GOAL API.',
  },
  quickFilters: {
    addAria: 'Ajouter un filtre rapide',
    confirmRemove: 'Voulez-vous vraiment retirer ce filtre ?',
    remove: 'Retirer',
  },
  clubDetail: {
    tabSquad: 'Effectif',
    tabFixtures: 'Matchs',
    tabTransfers: 'Transferts',
    squadUnavailable: "Effectif actuellement indisponible.",
    squadEmpty: 'Aucun effectif communiqué.',
    founded: (year) => `Fondé en ${year}`,
    capacity: (n) => `${n} places`,
    captain: 'Capitaine',
    injured: 'Blessé',
    noFixtures: 'Aucun match à venir.',
    noTransfers: 'Aucune actualité transfert pour ce club.',
  },
  errors: {
    notificationsDenied: "Les notifications n'ont pas été autorisées.",
    tabCrashed: 'Une erreur est survenue.',
    retry: 'Réessayer',
  },
};

const es = {
  nav: { news: 'Noticias', ligen: 'Ligas', live: 'En vivo', europa: 'Europa', settings: 'Ajustes', fixtures: 'Partidos', standings: 'Clasificación' },
  common: {
    loading: 'Cargando…',
    chooseClub: 'Elegir club…',
    notSupported: 'no compatible',
    vs: 'vs',
    cancel: 'Cancelar',
    refreshing: 'Actualizando…',
    releaseToRefresh: 'Soltar para actualizar',
    pullToRefresh: 'Desliza para actualizar',
  },
  transfers: {
    officialOnly: 'Solo traspasos oficiales',
    officialOnlyToggle: 'Alternar solo traspasos oficiales',
    empty: 'No hay noticias para esta selección.',
    official: 'oficial',
    rumor: 'rumor',
    searchPlayer: 'Buscar jugador',
    searchPlayerTitle: 'Buscar jugador en Transfermarkt',
    viewProfile: 'Ver perfil',
    viewProfileTitle: 'Ver perfil del jugador',
    readOriginal: 'Leer artículo',
    aiSummaryDisclaimer: 'Resumen generado por IA, puede contener errores. No sustituye al artículo original.',
    justNow: 'justo ahora',
    minutesAgo: (n) => `hace ${n} min`,
    hoursAgo: (n) => `hace ${n} h`,
    daysAgo: (n) => `hace ${n} día${n === 1 ? '' : 's'}`,
  },
  newsSection: { tabNews: 'News', tabTransfers: 'Traspasos' },
  fixtures: {
    currentMatchdayOnly: 'Solo jornada actual',
    currentMatchdayOnlyToggle: 'Alternar solo jornada actual',
    liveOnlyToggle: 'Alternar solo partidos en directo',
    todayOnlyToggle: 'Alternar solo partidos de hoy',
    empty: 'No hay partidos en el calendario.',
    matchday: (n) => `Jornada ${n}`,
    live: 'EN VIVO',
    today: 'HOY',
    finished: 'Finalizado',
    kickoffTbd: '-:-',
    favoriteAria: 'Marcar partido como favorito',
    unfavoriteAria: 'Quitar de favoritos',
    favoritedToast: 'Añadido a favoritos',
    unfavoritedToast: 'Eliminado de favoritos',
  },
  lineup: {
    notYetKnown: 'Alineación aún no conocida. Se publica en cuanto los clubes la confirman (normalmente 30–60 min antes del inicio).',
    official: 'Alineación oficial',
    predicted: 'Alineación probable',
    noPlayers: 'Aún no se han comunicado jugadores.',
    substitutes: 'Suplentes',
    coach: 'Entrenador',
    positions: { Goalkeeper: 'Portero', Defender: 'Defensa', Midfielder: 'Centrocampista', Forward: 'Delantero' },
  },
  playerProfile: {
    age: (n) => `${n} años`,
    statsAsOf: (d) => `Actualizado: ${d}`,
    goals: 'Goles',
    assists: 'Asistencias',
    yellowCards: 'Tarjetas amarillas',
    redCards: 'Tarjetas rojas',
    injured: 'Lesionado',
    noStats: 'No hay estadísticas disponibles.',
  },
  matchInfo: {
    tabLineups: 'Alineaciones',
    tabInfo: 'Info del partido',
    tabStats: 'Estadísticas',
    tabTable: 'Clasificación',
    tabHighlights: 'Highlights',
    noHighlights: 'Highlights aún no disponibles.',
    notFinished: 'La cronología de eventos estará disponible cuando termine el partido.',
    noEvents: 'Sin eventos destacados.',
    goal: 'Gol',
    ownGoal: 'Autogol',
    penalty: 'Penalti',
    yellowCard: 'Tarjeta amarilla',
    redCard: 'Tarjeta roja',
    substitution: 'Cambio',
    assistLabel: (name) => `Asistencia: ${name}`,
  },
  standings: {
    title: 'Clasificación',
    empty: 'Clasificación aún no disponible.',
    played: 'PJ',
    won: 'G',
    draw: 'E',
    lost: 'P',
    points: 'Pts',
    goalDiff: 'DG',
    zoneChampionsLeague: 'Champions League',
    zoneEuropaLeague: 'Europa League',
    zoneConferenceLeague: 'Conference League',
    zoneRelegationPlayoff: 'Playoff de descenso',
    zoneRelegation: 'Descenso',
  },
  europa: {
    tabSpiele: 'Partidos',
    tabTabelle: 'Clasificación',
    leaguePhase: 'Fase de Liga',
    zoneDirectQualify: 'Octavos de final directos',
    zonePlayoff: 'Ronda de playoffs',
    zoneEliminated: 'Eliminado',
    standingsEmpty: 'Clasificación no disponible todavía.',
  },
  live: {
    title: 'En vivo',
    empty: 'Ahora mismo no hay ningún partido en vivo',
    emptyHint: 'Vuelve más tarde, o consulta el calendario en Ligas o Europa.',
  },
  topscorers: {
    title: 'Máximos goleadores',
    empty: 'Clasificación de máximos goleadores aún no disponible.',
    goals: 'Goles',
    assists: 'Asis',
  },
  stats: {
    form: 'Forma',
    noForm: 'Aún no se han jugado partidos esta temporada.',
    headToHead: 'Enfrentamientos directos',
    noHeadToHead: 'Sin enfrentamientos esta temporada.',
    standing: 'Posición en la clasificación',
    positionLabel: (pos) => `${pos}º`,
    possession: 'Posesión',
    shotsOnGoal: 'Tiros a puerta',
    shotsOffGoal: 'Tiros fuera',
    shotsBlocked: 'Tiros bloqueados',
    corners: 'Córners',
    offsides: 'Fueras de juego',
    fouls: 'Faltas',
    yellowCards: 'Tarjetas amarillas',
    redCards: 'Tarjetas rojas',
    noStatistics: 'Aún no hay estadísticas para este partido.',
  },
  settings: {
    appearance: 'Apariencia',
    appearanceSystem: 'Sistema',
    appearanceLight: 'Claro',
    appearanceDark: 'Oscuro',
    accentColor: 'Color de acento',
    accentTerracotta: 'Terracotta',
    accentViolet: 'Violeta',
    accentGreen: 'Verde',
    accentMono: 'Mono',
    favoriteClub: 'Club favorito',
    quickFilters: 'Filtros rápidos',
    noQuickFilters: 'Aún no hay filtros rápidos. Añádelos en la pestaña Traspasos.',
    removeClub: (name) => `Eliminar ${name}`,
    notifications: 'Notificaciones',
    pushTransfers: 'Notificaciones push de noticias de fichajes',
    pushTransfersToggle: 'Alternar notificaciones de noticias de fichajes',
    pushLineups: 'Notificaciones push de alineaciones oficiales',
    pushLineupsToggle: 'Alternar notificaciones de alineaciones oficiales',
    language: 'Idioma',
    legal: 'Aspectos legales',
    imprint: 'Aviso legal',
    privacyPolicy: 'Privacidad',
    dataSource: 'Fuentes de datos',
    dataSourceAttribution: 'Football data provided by the Football-Data.org API. Otros datos: GOAL API.',
  },
  quickFilters: {
    addAria: 'Añadir filtro rápido',
    confirmRemove: '¿Seguro que quieres eliminar este filtro?',
    remove: 'Eliminar',
  },
  clubDetail: {
    tabSquad: 'Plantilla',
    tabFixtures: 'Partidos',
    tabTransfers: 'Fichajes',
    squadUnavailable: 'Plantilla no disponible por ahora.',
    squadEmpty: 'Sin plantilla disponible.',
    founded: (year) => `Fundado en ${year}`,
    capacity: (n) => `${n} asientos`,
    captain: 'Capitán',
    injured: 'Lesionado',
    noFixtures: 'Sin próximos partidos.',
    noTransfers: 'Sin noticias de fichajes para este club.',
  },
  errors: {
    notificationsDenied: 'No se permitieron las notificaciones.',
    tabCrashed: 'Algo salió mal.',
    retry: 'Reintentar',
  },
};

const TRANSLATIONS = { de, en, it, fr, es };

export function getTranslations(language) {
  return TRANSLATIONS[language] || TRANSLATIONS.de;
}
