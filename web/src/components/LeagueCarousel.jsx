...
// (each tab already has one, e.g. TransfersTab's `ref={scrollRef}` list) --
// interactive bits tied to app-level state (overlays, filters) stay in the
// calling tab and are deliberately not passed a league override, since the
// preview panel represents "what you're about to land on", not something
// meant to be interacted with mid-drag.
export default function LeagueCarousel({ league, onSwitchLeague, renderPage }) {
  const { containerRef, offsetX, direction, settling } = useLeagueCarousel((dir) => onSwitchLeague(dir === 'next' ? 1 : -1));
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', flex: 1, minHeight: 0, overflow: 'hidden' }}>
      ...
      {direction && (
        ...
        >
          {renderPage(adjacentLeague(league, direction === 'next' ? 1 : -1).slug)}
        </div>
      )}
    </div>
  );
