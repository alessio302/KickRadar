  ...
  return (
    ...
    >
      ...
      {favoritable ? (
        ...
        >
          <Star size={15} fill={isFavorite ? FAVORITE_STAR_COLOR : 'none'} color={isFavorite ? FAVORITE_STAR_COLOR : theme.textMuted} />
        </button>
      ) : (
        ...
        // spacer took up a genuine 24px while the button's negative margin
        // shrank its own effective footprint by 6px, so every column after
        // this one sat 6px further right on a finished row than on a
        // favoritable one.
        <span style={{ width: '24px', margin: '0 -3px', flex: '0 0 auto' }} aria-hidden="true" />
      )}
    </div>
  );
}
