...
// Favoriting a fixture (swipe action on its card, see FixtureRow.jsx) gets
// you push notifications for its live events (goals/cards/subs, see
// src/lineups/matchEventNotifier.js). Tied to the push subscription
// itself, same as every other push preference in this app -- there's no
// login system, so "favorite" only means anything once a subscription
// exists; toggling a favorite silently creates one first (same
// permission-prompt flow as the Settings toggles) rather than failing
// outright.
export function useFavoriteFixtures(language) {
