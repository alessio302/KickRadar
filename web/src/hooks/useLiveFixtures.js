...
// `fixtures` with no league_id filter -- unlike useFixtures.js's own
// per-league query. Clubs and leagues are joined client-side, same
...
//
// Domestic-only (see the `.not('home_club_id', ...)` filter below) --
// useEuropaFixtures() already has its own live UEFA fixtures (team_name-
// keyed, no clubs table row), so LiveTab.jsx's own useAllLiveFixtures()
// combines this hook's output with that one instead of this file trying to
// cover both shapes itself.
const load = async (setFixtures, setLoading, cancelledRef) => {
  const [{ data: liveRows, error: fixturesErr }, { data: clubs, error: clubsErr }, { data: leagues, error: leaguesErr }] =
    await Promise.all([
      ...
      supabase
        ...
        // 2026-09-09. useAllLiveFixtures.js's own useEuropaFixtures() half
        // already has its own live UEFA fixtures.
...
export function useLiveFixtures() {
  ...
  const refetch = useCallback(() => {
    ...
    return load(setFixtures, setLoading, cancelledRef);
  }, []);

  useEffect(() => {
    const cancelledRef = { current: false };
    load(setFixtures, setLoading, cancelledRef);

    // A fixture entering or leaving the live set changes this list's whole
    // membership, not just a field on an already-known row (unlike
    ...
    const channel = supabase
      .channel('live-fixtures-domestic')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'fixtures' }, () => {
        if (!cancelledRef.current) load(setFixtures, setLoading, cancelledRef);
      })
      .subscribe();
    ...
    function handleWake() {
      if (!cancelledRef.current && document.visibilityState === 'visible') load(setFixtures, setLoading, cancelledRef);
    }
